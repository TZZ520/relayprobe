...
最理想的检测方式，是用同一组合成探针分别跑“官方 API baseline”和“待测中转 API”，再比较两边在参数约束、流式协议、JSON 格式、usage 字段、模型字段、错误形态等方面的差异。

- 高可信度：有官方 API 对照结果，多轮重复结果稳定，并且多个独立检测项同时指向同一类异常。
- 中等可信度：没有官方 API 对照，但出现了明确的协议、参数、nonce、usage 或响应改写异常；这类结果适合作为风险筛查和复测线索。
- 低可信度：只有单次运行结果，或只依赖模型自称、自然语言回答、返回的 `model` 字段等弱证据。

如果没有官方 API baseline，relayprobe 的结果仍然有价值，但应理解为“黑盒可观察证据”，不能单独作为最终定性或公开指控。对于严肃结论，建议补跑官方 API、自托管直连模型或可信上游的对照组。
